...
fun UnlockRoot() {
    ...
    val strings = if (lang == "ru") Strings.ru() else Strings.en()
    ...
    CompositionLocalProvider(LocalStrings provides strings) {
        ...
        Scaffold(
            topBar = {
                TopAppBar(
                    title = {
                        Text(current?.let { if (it == Destination.Dashboard) strings.appName else it.title(strings) } ?: strings.appName)
                    },
                    actions = {
                        IconButton(onClick = {
                            nav.navigate(Destination.Settings.route) { launchSingleTop = true }
                        }) {
                            Icon(Icons.Filled.Settings, contentDescription = strings.navSettings)
                        }
                    },
                )
            },
                ...
                NavigationBar {
                    Destination.bottom.forEach { dest ->
                        NavigationBarItem(
                            ...
                            icon = { Icon(dest.icon, contentDescription = dest.title(strings)) },
                            label = { Text(dest.title(strings)) },
                        )
                    }
                }
            },
